...
if [[ ${1} == "screenshot" ]]; then
    SERVICE_IP="http://$(ping -c1 -4 service | sed -nE 's/^PING[^(]+\(([^)]+)\).*/\1/p'):8787/system/status"
    NETWORK_IDLE="2"
...
EOF
    ...
    [[ -z ${version} ]] && exit 1
    find . -type f -name '*.Dockerfile' -exec sed -i "s/ARG READARR_VERSION=.*$/ARG READARR_VERSION=${version}/g" {} \;
